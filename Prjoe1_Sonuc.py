...
img=cv2.imread('staj_shape8.jpg')
...
_,threshold=cv2.threshold(gray1,90,255,cv2.THRESH_BINARY)#treshold çıkardık
...
for cnt in sorted_contours:#kontör kümeleri içinde for döngüsü oluşturduk
    img1 = cv2.imread('staj_shape8.jpg',cv2.IMREAD_GRAYSCALE)#resimi tekrar içe aktardık grayscale uyguladık
    scale_percent = 70
    width = int(img1.shape[1] * scale_percent / 100)
    height = int(img1.shape[0] * scale_percent / 100)
    dim = (width, height)  
    img1 = cv2.resize(img1, dim, interpolation = cv2.INTER_AREA)#boyutunu ayarladık
    img1 = cv2.blur(img1,(5,5))#gauss filtresi uyguladık
    ...
    if size <100000:#100000 den küçük kontör sayısına sahip kontör kümelerini aldık böylece sadece şekiller üzerinde işlem yap
        ...
        lines = cv2.HoughLinesP(edges, 2, np.pi/180, 60, np.array([]), 50, 10)#şekilde ki düz çizgileri bulduk
